...
// stages that mean the candidate got past "just applied" into a real interaction
const INTERVIEW_STAGES = new Set(["screening", "assessment", "interview", "offer"]);

export default function StatsSummary({ rows }: Props) {
  const total = rows.length;
  if (total === 0) return null; // nothing to summarize yet

  const advancing = rows.filter((r) => r.status === "Advancing").length;
  const rejected = rows.filter((r) => r.status === "Rejected").length;
  const responded = advancing + rejected; // a human actually decided or moved you forward
  const responseRate = Math.round((responded / total) * 100);
  const reachedInterview = rows.filter(
    (r) => r.timeline?.some((t) => INTERVIEW_STAGES.has(t.stage))
  ).length;
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
      {stats.map((s) => (
        <div key={s.label} className={`bg-white rounded-xl border ${s.ring} p-4`}>
