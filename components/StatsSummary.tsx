...
export default function StatsSummary({ rows, onFilterStatus, onFilterInterviewed }: Props) {
  ...
  const stats: {
    label: string; value: string; accent: string; ring: string; onClick?: () => void;
  }[] = [
    { label: "Applications", value: String(total), accent: "text-gray-900", ring: "border-gray-200", onClick: () => onFilterStatus("All") },
    { label: "Response rate", value: `${responseRate}%`, accent: "text-indigo-600", ring: "border-indigo-100" },
    { label: "Reached interview", value: String(reachedInterview), accent: "text-blue-600", ring: "border-blue-100", onClick: onFilterInterviewed },
    { label: "Advancing", value: String(advancing), accent: "text-green-600", ring: "border-green-100", onClick: () => onFilterStatus("Advancing") },
    { label: "Rejected", value: String(rejected), accent: "text-red-600", ring: "border-red-100", onClick: () => onFilterStatus("Rejected") },
  ];
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5 sm:gap-3 mb-6">
      {stats.map((s) => (
        <button
          ...
          onClick={s.onClick}
          disabled={!s.onClick}
          className={`text-left bg-white rounded-xl border ${s.ring} p-3 sm:p-4 transition ${
            s.onClick ? "hover:border-gray-300 hover:shadow-sm cursor-pointer" : "cursor-default"
          }`}
        >
