...
// Group skills into the three CV buckets, in CV order.
function skillGroups(skills?: { name: string; group: string }[]) {
  const g = (k: string) => (skills || []).filter((s) => s.group === k).map((s) => s.name);
  return [
    { label: "Programming Languages", items: g("language") },
    { label: "Technologies", items: g("technology") },
    { label: "Soft Skills", items: g("soft") },
...
export function buildMarkdown(d: ProfileData): string {
  ...
  const sg = skillGroups(d.skills);
  if (sg.length) {
    lines.push("", "## Skills");
    for (const grp of sg) lines.push("", `**${grp.label}:** ${grp.items.join(", ")}`);
  }
...
export async function exportPdf(d: ProfileData) {
  ...
  const sgPdf = skillGroups(d.skills);
  if (sgPdf.length) {
    heading("Skills");
    for (const grp of sgPdf) {
      line(`${grp.label}: ${grp.items.join(", ")}`, 10, false, 5, "#333333");
    }
  }
...
export async function exportDocx(d: ProfileData) {
  ...
  const sgDoc = skillGroups(d.skills);
  if (sgDoc.length) {
    H("Skills");
    for (const grp of sgDoc) {
      kids.push(new Paragraph({ children: [
        new TextRun({ text: `${grp.label}: `, bold: true, size: 20 }),
        new TextRun({ text: grp.items.join(", "), size: 20 }),
      ] }));
    }
  }
